...
export const PropertyForm: React.FC = () => {
  ...
  const inputClass = 'w-full bg-[#FAF8F7] text-sm px-3.5 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#E51821]';
  const label = 'block text-xs font-bold text-[#1A1818] mb-1.5';

  if (sent) {
    return (
      <div className="bg-white rounded-3xl border border-gray-200/80 shadow-md p-10 text-center">
        <div className="w-16 h-16 bg-[#FFF0F1] text-[#E51821] rounded-full flex items-center justify-center mx-auto">
          <CheckCircle2 className="w-10 h-10" />
        </div>
        <h3 className="font-heading font-extrabold text-2xl text-[#1A1818] mt-5">Property Submitted</h3>
        <p className="text-[#6B655C] mt-2 max-w-md mx-auto">Thank you. Our site-selection team will review your property against our location criteria and reach out if it's a potential fit.</p>
        <button onClick={() => setSent(false)} className="mt-6 bg-[#E51821] text-white font-bold px-6 py-3 rounded-xl text-sm cursor-pointer">Submit Another</button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="bg-white rounded-3xl border border-gray-200/80 shadow-md p-6 sm:p-8 space-y-5">
      ...
      <label className="flex items-center gap-2 text-sm text-[#6B655C] border-2 border-dashed border-gray-300 rounded-xl p-3 cursor-pointer hover:border-[#E51821] transition-colors">
        <Upload className="w-4 h-4 text-[#E51821]" />
        <span>Attach property images / documents — upload wiring ready for backend</span>
        <input type="file" multiple className="hidden" />
      </label>

      <label className="flex items-start gap-2 text-xs text-[#6B655C]">
        <input type="checkbox" required className="mt-0.5 accent-[#E51821]" />
        <span>I consent to Fri-Chiks ® reviewing and storing these property details for site evaluation.</span>
      </label>

      <button type="submit" className="w-full bg-[#E51821] hover:bg-[#B80F16] text-white font-bold py-3.5 rounded-xl text-base transition-all flex items-center justify-center gap-2 cursor-pointer">
        <Send className="w-4 h-4 text-[#FFC700]" /> Submit Property
      </button>
    </form>
  );
};
