  ...
  return (
    <>
      <PageHero
        ...
        title={<>Partner in Our <span className="text-[#FFC700]">Supply Chain</span></>}
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-12 flex flex-wrap justify-center gap-3">
            {categories.map((c) => (
              <span key={c} className="flex items-center gap-2 bg-[#FAF8F7] border border-gray-200/80 rounded-full px-4 py-2 text-sm font-semibold text-[#1A1818]">
                <CheckCircle2 className="w-4 h-4 text-[#E51821]" />{c}
              </span>
            ))}
          </div>
        </div>
            ...
            {standards.map((s) => {
              ...
              return (
                <div key={s.title} className="bg-white rounded-2xl p-6 border border-gray-200/80 text-center">
                  <div className="w-12 h-12 rounded-2xl bg-[#FFF0F1] text-[#E51821] flex items-center justify-center mx-auto mb-4"><Icon className="w-6 h-6" /></div>
                  <h3 className="font-heading font-bold text-base text-[#1A1818]">{s.title}</h3>
                  <p className="text-sm text-[#6B655C] mt-1.5">{s.text}</p>
                </div>
              );
            })}
          </div>
